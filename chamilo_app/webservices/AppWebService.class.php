...
class AppWebService extends WSAPP
{
        ...
        foreach ($listSessions as $cat_session) {
            ...
            foreach($cat_session['sessions'] as $sessions){
                ...
                foreach($sessions['courses'] as $course_session){
                    ...
                    $info_course_session['url_picture'] = $infoCourse['course_image_large'];
                    $info_course_session['course_image'] = $infoCourse['course_image'];
                ...
                $list_sessions[] = array(
                    ...
                    'access_start_date'    => api_format_date($sessions['access_start_date'], DATE_TIME_FORMAT_SHORT),
                    'access_end_date'    => api_format_date($sessions['access_end_date'], DATE_TIME_FORMAT_SHORT),
                    'courses'    => $list_courses
                );
            }
             $list_categories[] = array(
                'session_category_id' => $cat_session['session_category']['id'],
                'session_category_name' => $cat_session['session_category']['name'],
     ...
     */
    public function registerAccessCourse($c_id, $user_id, $s_id = 0)
    {
        ...
        $section = array();
        while ($row = Database::fetch_assoc($result)) {
            $section[$row['name']] = $row['visibility'];    
        }
        ...
        $sql = "SELECT * FROM $course_tool_table t
                        WHERE category = 'plugin' AND c_id = $c_id $condition_session
                        ORDER BY id";
        $result = Database::query($sql);
        while ($row = Database::fetch_assoc($result)) {
            $section[$row['name']] = $row['visibility'];    
        ...
        foreach ($categories as $item) {
            ...
            if (!empty($flat_list)) {
                foreach ($flat_list as $id => $details) {
                    ...
                    $url_start_lp = api_get_cidreq().'&action=view&lp_id='.$id;
     ...
     */
    public function getDocuments($c_id, $path, $user_id, $s_id)
    {    
        /* LOGIN */
        $courseInfo = api_get_course_info_by_id($c_id);
        $chamiloUser = api_get_user_info($user_id);
    ...
    {    
        ...
        foreach ($events as $row) {
            ...
            $results[] = array(
               ...
               'title' => $row['title'],
               'content' => $description, 
               'start_date' => $row['start_date_localtime'], 
               'end_date' => $row['end_date_localtime'],
               'all_day' => $row['allDay']
               );
        }
        return $results;
